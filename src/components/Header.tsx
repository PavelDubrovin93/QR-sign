...
function Header({ nav }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    if (currentItem) {
      ...
      setSelectedId(nav[0]?.id || null);
    }
  }, [nav]);

  const handleItemClick = (item: NavItem) => {
    setSelectedId(item.id);
    navigate(item.path_tab);
  };

  const getItemStyle = (isSelected: boolean) => {
    if (telegramData?.colorScheme !== "dark") return {};

    return {
      backgroundColor: isSelected
        ? telegramData?.themeParams.header_bg_color
        : telegramData?.themeParams.section_bg_color,
    };
  };

  useEffect(() => {
    if (current_role === Roles.EMPLOYER || current_role === Roles.FOREMAN) {
