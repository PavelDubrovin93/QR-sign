...
class UISettingsRepository(IUISettingsRepository):
    ...
    async def get_ui_settings_by_user_id(self, user_id: int) -> Optional[UISettingsDTO]:
    ...
    async def create_ui_settings(
        self, settings_data: UISettingsDTO
    ...
    async def update_ui_settings(
        ...
        query = select(UISettingsEntity).where(
            UISettingsEntity.user_id == settings_data.user_id
        )
        result = await self.session.execute(query)
        ...
        ui_settings_dto = (
            await self.__to_dto(existing_settings) if existing_settings else None
        )
        return ui_settings_dto
