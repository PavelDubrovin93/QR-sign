...
from app.infrastructure.interfaces.services.ITaskBoardService import \
    ITaskBoardService
...
from app.validation.responses.TaskBoardResponse import (
    CreateTaskBoardResponse, TaskBoardResponse, TaskPointResponse)
...
class TaskBoardService(ITaskBoardService):
    ...
    async def edit_task_board_with_task_points(self, taskboard: TaskBoardResponse):
        ...
        new_task_points = await self.tp_repo.edit_task_points_by_dto_list(
            task_points=taskboard.task_points, task_board=taskboard.id
        )
        taskpoints_to_response = []
    ...
    async def get_task_boards_by_company_id_and_user_id(
        ...
        taskboards = await self.tb_repo.get_task_boards_by_company_id_and_user_id(
    ...
    async def create_taskboard(
        ...
        for task_point in taskboard_data.task_points:
            new_taskpoint = await self.tp_repo.add_task_point(
                new_task_point=TaskPointDTO(
    ...
    async def get_taskboard_by_work_group_id(
        self, work_group_id: int
    ) -> List[TaskBoardDTO]:
        taskboard = await self.tb_repo.get_task_board_by_work_group_id(
